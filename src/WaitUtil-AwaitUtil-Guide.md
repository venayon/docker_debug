...
| Anything **not on the page** — an API, database, queue, file, async job | [`AwaitUtil`](#awaitutil-backend--async-state) |

If your step touches `WebDriver` or `WebElement`, it's `WaitUtil`.
If your step touches a `RestTemplate`, a repository, a message client, or
`Files.exists(...)`, it's `AwaitUtil`.
